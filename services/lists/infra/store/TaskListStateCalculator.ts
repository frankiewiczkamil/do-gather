...
export function calculateTaskListState(taskListEvents: Array<TaskListEventFailed | TaskListSuccessEvent>, taskListId: TaskListIdentifier) {
  return taskListEvents.filter(isApplicableEventFactory(taskListId)).reduce(applyTaskListEvent, createTaskListBase(taskListId));
}
